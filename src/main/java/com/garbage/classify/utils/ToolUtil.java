...
//	}
    ...
    /**
     * 匹配FeatureList是否满足条件FeatureList
     *
     * @param featureList
     * @param split
     * @param recommendConfigAmount
     * @return
     */
    public static boolean featureMatch(List<String> featureList, List<String> split, Integer recommendConfigAmount) {
        Integer match = 0;
        for (String fea : split) {
            if (featureList.contains(fea)) {
                match += 1;
                if (match >= recommendConfigAmount) {
                    //匹配该规则
                    return true;
                }
            }
